...
class device:
    TIMEOUT = 3600    # Number of seconds before command times out
    ...
    def __init__(self, newUDID, tuple):
        ...
        self.queue = deque()
        self.last_command_queue = deque()
        print "device object initialized"
    ...
    def sendCommand(self):
        ...
        # pushing the last command to old command queue
        print "Length of last_command_queue == "
        print self.last_command_queue
        print "Pushing the command to last_command_queue"
        print cmd
        self.last_command_queue.append(cmd)
        print "And quey after pushing"
        print self.last_command_queue
    ...
    def reverseCommand(self):
        if len(self.last_command_queue) == 0:
            print "**No commands left in queue"
            return ''
        
        cmd = self.last_command_queue.popleft()
        print "Pulling the last command & pushing it"
        print cmd
        self.queue.append(cmd)
        print "Printing the queue"
        print self.queue
        
    def popCompletedCommand(self):
        HIGH='[1;31m'
        LOW='[0;32m'
        NORMAL='[0;39m'
        if len(self.last_command_queue) == 0:
        ...
        cmd = self.last_command_queue.popleft()
        print HIGH+" Popping Command " + cmd['Command']['RequestType'] + " " +NORMAL
        print "Removed the last cammand from last_command_queue: "
        print cmd        
    ...
    def checkTimeout(self):
        ...
        for commandUUID in self.cmdList:
            if self.cmdList[commandUUID]['response'] == "" and now-self.cmdList[commandUUID]['cmd']['TimeStamp'] > self.TIMEOUT:
                print "Going in danger"                  
                self.status = 2
                self.cmdList[commandUUID['cmd']['CommandUUID']]['status'] = 'danger'
                self.cmdList[commandUUID['cmd']['CommandUUID']]['response'] = {'Status':'TimeoutError'}
